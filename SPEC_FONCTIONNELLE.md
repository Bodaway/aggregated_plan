...
| **R37** | **Horizon de matérialisation** : les instances récurrentes sont générées de manière paresseuse sur un horizon glissant de 14 jours à compter d'aujourd'hui. Aucune instance n'est pré-créée au-delà de cet horizon. La génération est déclenchée à chaque consultation de la liste de tâches, de la matrice de priorité, du dashboard, après chaque synchronisation externe, et par un job de fond horaire (R83). Elle ne crée **jamais** d'occurrence antérieure à aujourd'hui : une occurrence ratée est un fait historique, pas quelque chose à recréer — un modèle dont la génération s'est arrêtée il y a quatre mois ne rejoue pas ces quatre mois. La génération est idempotente : tenter de créer une instance déjà existante pour un (modèle × date) n'a aucun effet. |
...
| **R80** | **Effondrement à l'affichage** : une série récurrente ne contribue qu'une seule ligne aux listes de tâches — l'occurrence de date d'occurrence maximale parmi celles à ou avant aujourd'hui. Les occurrences futures de l'horizon de matérialisation sont masquées, et une série dont toutes les occurrences sont à venir ne contribue rien : la liste répond à « qu'est-ce qui est à faire maintenant ». L'effondrement choisit sa ligne **sans regarder le statut** ; le filtre de statut demandé s'applique ensuite à la ligne retenue, si bien qu'une série dont l'occurrence courante est terminée ne remonte pas sur un filtre `todo`. Les tâches non récurrentes sont hors du champ de la règle. La recherche en est exclue : elle répond à « où ai-je vu ça » et doit retrouver une occurrence passée. Un appelant qui veut la série entière le demande explicitement (`allOccurrences`, ou `aplan ls --all-occurrences`). Le dédoublonnage propre à la matrice de priorité (R39) devient de ce fait redondant sans devenir faux : la matrice ne reçoit plus qu'une ligne par série. |
| **R81** | **Balayage des occurrences périmées** : une occurrence dont la date est antérieure à aujourd'hui et dont le statut n'est ni terminé ni annulé passe automatiquement au statut `annulée`. Le balayage atteint aussi les instances des modèles désactivés — annuler une série ne retire pas les occurrences qu'elle a déjà produites. **Exception** : une occurrence portant au moins une entrée de worklog n'est jamais balayée. L'annuler la ferait disparaître de toutes les vues de tâches alors qu'elle porte la trace d'un travail réel, et la rouvrir à la main ne servirait à rien puisque le passage suivant la refermerait. Le statut `annulée` recouvre donc deux faits distincts — une occurrence sautée volontairement (R33) et une occurrence périmée — et l'interface peut libeller la seconde « abandonnée » sans que ce soit un statut supplémentaire. |
...
| **R69** | **Deux ou trois boutons, sept issues, dont deux volontairement distinctes**. La notification porte toujours *Prendre la pause* et *Passer*, et *Plus tard* seulement quand la cadence de la règle l'autorise (R71) ; chaque pause aboutit à l'une des sept issues suivantes : (1) `taken` — la pause a été ouverte **et menée à son terme** : depuis R78, ce n'est plus le clic qui l'écrit mais l'échéance atteinte ; (2) `abandoned` — la pause a été ouverte puis coupée avant son terme (R79) ; (3) `snoozed` — l'utilisateur a cliqué *Plus tard*, un report de suivi est armé (issue impossible sur une règle infra-horaire, qui n'offre pas le bouton) ; (4) `skipped` — l'utilisateur a cliqué *Passer*, refus délibéré de cette occurrence ; (5) `ignored` — la notification s'est fermée sans qu'un choix soit fait ; (6) `absorbed` — effacée par la fusion des collisions (R68), jamais vue par l'utilisateur ; (7) `expired` — ne pouvait plus utilement se déclencher (R67), ou n'a pas pu être affichée du tout, faute d'écran (API sans session graphique) : dans les deux cas la pause n'a jamais atteint l'utilisateur, et l'enregistrer en `ignored` ferait dire à la statistique qu'il a ignoré une pause qu'il n'a jamais vue. `skipped` et `ignored` sont délibérément distincts : ignorer systématiquement la pause de 15 minutes signale une cadence mal réglée, alors que la passer explicitement signale un timing mal choisi pour cette occurrence précise — deux corrections différentes. |
| **R70** | **Statistique d'adhérence, deux issues exclues des deux côtés**. Le taux d'adhérence par règle est `pris / vus`, où « vus » ne compte que les issues que l'utilisateur a effectivement pu voir (`taken` + `abandoned` + `snoozed` + `skipped` + `ignored`) ; `absorbed` et `expired` sont exclus **du numérateur et du dénominateur** — la pause n'a jamais atteint un écran, la compter noierait un signal réel dans le bruit de planification. `abandoned` est du même côté que `skipped` ou `ignored` et non de celui du bruit : la pause a bien atteint l'utilisateur, elle n'est simplement pas allée au bout (R79). Le cas « rien de vu » rend un taux **absent**, jamais zéro, pour ne pas afficher une adhérence nulle là où la règle n'a simplement pas encore eu l'occasion de se manifester. |
| **R71** | **Une pause qui revient plus souvent que toutes les heures ne se reporte pas** : elle se prend ou elle se passe. En dessous de l'heure, la notification ne porte que *Prendre la pause* et *Passer* ; à partir de 60 min, et pour toute règle quotidienne, les trois boutons restent. La raison est un effet de composition, pas une question de goût : reporter une cadence courte ne déplace pas la pause, cela en ajoute une. Le report (10 min par défaut) retombe à l'intérieur d'un intervalle déjà court — sur une règle de 15 min, l'utilisateur reçoit le report puis l'échéance suivante de la grille dans les cinq minutes qui suivent —, là où sur une règle horaire il reste cinquante minutes de marge avant l'échéance suivante. C'est ce cumul qui a rendu la routine inutilisable dès son premier après-midi : neuf notifications en soixante-quatre minutes, dont deux dans la même minute, une seule pause horaire ayant produit à elle seule quatre notifications à force d'être reportée. Une règle quotidienne n'a qu'une échéance dans la journée et ne peut se cumuler avec rien : elle garde le report. La restriction vaut aussi côté écriture — une action `snoozed` qui arriverait malgré tout sur une règle infra-horaire (démon de notification rejouant une action périmée) est enregistrée en `skipped`, sans report de suivi, plutôt que de ressusciter le comportement qu'on vient de retirer. |
| **R78** | **Presser le bouton ouvre une pause, il ne l'enregistre pas**. Le premier bouton de la notification s'intitule désormais *Prendre la pause* et le presser **ouvre une session de pause** au lieu de clore l'échéance : l'instant de la pression devient le départ de la pause, la fin est **figée** à ce départ plus la durée de la règle, et l'issue reste `pending`. L'overlay HUD est montré et affiche l'écran de pause — un anneau de progression qui se vide, le temps restant en `m:ss`, l'intitulé et le corps de la règle (ce qu'il faut faire concrètement), et un seul bouton, *J'y retourne*. Pas de rallonge : prolonger une pause n'est le problème de personne, et chaque contrôle supplémentaire serait un cas de plus à tenir. À l'échéance, l'issue passe à `taken`, l'overlay se referme et une notification *Pause terminée* d'urgence basse l'annonce à voix haute : un overlay qui disparaît est un signal muet, et le principe d'une pause est justement qu'on ne regarde pas l'écran. La fin est figée à l'ouverture et jamais recalculée depuis la règle, pour que régler la durée dans l'écran de réglages pendant une pause ne rallonge pas la pause en cours ; c'est aussi le seul instant absolu, lu à la fois par le backend et par l'écran, si bien qu'aucun des deux ne peut dériver par rapport à l'autre. Le passage de la routine **garde la main pendant toute la pause**, ce qui a une conséquence voulue : aucune pause ne peut sonner pendant une pause. Sans session graphique, la pause se déroule et s'enregistre quand même — le décompte appartient au backend —, elle n'a simplement rien à montrer. *Plus tard*, *Passer*, une notification fermée sans réponse ou non délivrée n'ouvrent aucune session et se comportent exactement comme avant. **Ce que cette règle corrige** : `taken` enregistrait une *intention* — l'instant où la popup avait été fait taire —, la durée prescrite était purement décorative, et une « pause franche de cinq minutes » se soldait en pratique par trois secondes et un clic. |
| **R79** | **Couper une pause est un échec mesuré, pas du bruit**. *J'y retourne*, ou toute fin anticipée, écrit la septième issue, `abandoned`, qui **compte dans l'adhérence des deux côtés** au même titre que `snoozed`, `skipped` et `ignored` : la notification a été vue, l'utilisateur y a répondu, la pause n'est pas allée au bout. C'est l'inverse d'`absorbed` et d'`expired`, qui n'ont jamais atteint un écran et restent exclus des deux côtés (R70). Le panneau d'adhérence affiche `abandoned` sur sa propre ligne : sans cette colonne, la ventilation ne se recompose plus en le taux qu'elle surmonte, et c'est précisément l'échec que le panneau existe pour rendre visible. **La course est tranchée en faveur du décompte** : si l'échéance tombe dans la seconde où le bouton est pressé, c'est le `taken` du décompte qui tient — avoir vu passer l'échéance fait autorité sur la façon dont la pause s'est terminée ; un second appui, ou un appui portant sur une pause déjà close, ne réécrit rien et le dit. **Redémarrage de l'API pendant une pause** : au passage suivant, une session dont la fin est **déjà passée** est close en `taken` — le décompte était lancé et il est allé à son terme tout seul, l'appeler autrement serait inventer un abandon que rien n'atteste — tandis qu'une session encore en cours est **laissée à son propriétaire** et sera revue à un passage ultérieur. Ne rien décider sur une session vivante coûte au plus un passage et évite deux faux abandons : celui qu'un second processus API ouvert sur la même base fabriquerait sur une pause parfaitement saine, et celui qu'un recul d'horloge produirait — un abandon comptant contre l'adhérence, un simple redémarrage en pleine pause salissait la statistique. |
...
Un rythme de la routine. C'est ce que l'écran de réglages liste et édite. Voir R65 à R70 et R78.
...
| duréeSecondes | Entier | Oui | Durée **réelle** de la pause : c'est elle que décompte l'écran de pause et elle qui fige la fin de la session (R78). Entre 1 et 3600 s ; le passage attend la notification puis toute la pause en ligne, une valeur absurde immobiliserait la routine |
...
Une échéance. C'est la trace de ce qui s'est passé, et ce qui permet à un report — comme à une pause en cours — de survivre à un redémarrage de l'API. Voir R65 à R70, R78 et R79.
...
| issue | Enum | Oui | `pending`, `taken`, `snoozed`, `skipped`, `ignored`, `absorbed`, `expired`, `abandoned` (R69) |
| réponduLe | Date/heure | Non | |
| démarréeLe | Date/heure | Non | Instant où l'utilisateur a pressé *Prendre la pause* ; `null` tant qu'aucune session n'a été ouverte (R78) |
| finPrévue | Date/heure | Non | Fin de la pause, figée à `démarréeLe + duréeSecondes` à l'ouverture de la session et jamais recalculée ensuite (R78) |
