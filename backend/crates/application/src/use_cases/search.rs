...
use crate::services::{MemoryRetriever, RecallQuery};
...
pub async fn search(
    ...
    let mut tasks: Vec<SearchHit> = task_repo
        ...
        .into_iter()
        .filter(|t| {
            matches(&t.title, &terms) || t.description.as_deref().is_some_and(|d| matches(d, &terms))
        })
        .map(|t| SearchHit {
            id: t.id.to_string(),
            title: t.title,
    ...
    let memories = recall_hits(memory_retriever, user_id, &request.query, request.limit, now).await?;
